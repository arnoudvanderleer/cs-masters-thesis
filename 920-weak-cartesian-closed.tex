...
\begin{definition}
  Let $ \C $ be a category with binary products (we will call the projections $ \pi_1 $ and $ \pi_2 $), and take $ A, B : \C $. A \iindex{semi-exponential object} is an object $ A^B : \C $, with a morphism $ \mathrm{ev}: \C(A^B \times B \to A) $ and a function
...
\begin{definition}
  A \iindex{reflexive object} in a category $ \C $ with binary products is an object $ U $ with a semi-exponential $ U^U $ and morphisms $ f $ and $ g $, forming the following commutative diagram:
  \begin{center}
    \begin{tikzcd}
