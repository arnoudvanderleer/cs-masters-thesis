...
\begin{document}
  ...
  \[ T(n) = F(G(\{1, \dots, n\})). \]
  Given $ c \in \Ob{\CC} $, we have a bijection $ \varphi: \Hom{\SET}{\{1, \dots, n\}}{F(c)} \xrightarrow{\sim} \Hom{\CC}{G(\{1, \dots, n\})}{c} $. Then, for $ f \in T(m) $, $ g: \{ 1, \dots, m \} \to T(n) $, we can set $ f \bullet g := F(\varphi(g))(f) $. We also take $ \pi_{n, i} = \varphi^{-1}(\mathrm{id}_{G(\{1, \dots, n\})})(i) $. Then, given any $ A \in \Ob \CC $, we can make $ F(A) $ into an algebra by setting $ \alpha_n(f, a) = F(\varphi(a))(f) $. Also, given a morphism $ f \in \Hom{\CC}{A}{B} $, we have a function $ F(f) \in \Hom{\SET}{F(A)}{F(B)} $ (\textbf{TODO: Show that it is an algebra morphism}). Of course, $ F $ respects composition and identities, so we have a functor from objects of $ \CC $ to $ T $-algebras.
