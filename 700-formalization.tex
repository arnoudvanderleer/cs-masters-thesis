...
% PresheafCategory.v PresheafCategoryCore.v PresheafMorphisms.v Presheaves.v
  \item Definitions for algebraic theories (\coqident{AlgebraicTheories.AlgebraicTheories}{algebraic_theory}), $ \lambda $-theories (\coqident{AlgebraicTheories.LambdaTheories}{lambda_theory}), algebras (\coqident{AlgebraicTheories.Algebras}{algebra}) and presheaves (\coqident{AlgebraicTheories.Presheaves}{presheaf}) of algebraic theories, together with their morphisms and their categories. Proofs that the categories are univalent, that the categories of algebraic theories, $ \lambda $-theories and presheaves have limits and that the categories of algebras and presheaves are fibered over the category of algebraic theories.
% FreeTheory.v LambdaCalculus.v OnePointTheory.v ProjectionsTheory.v TheoryAlgebra.v
  \item The terminal algebraic theory (\coqident{AlgebraicTheories.Examples.OnePointTheory}{one_point_theory}), the free algebraic theory on a set (\coqident{AlgebraicTheories.Examples.FreeTheory}{free_functor}), with as a special case the initial algebraic theory (\coqident{AlgebraicTheories.Examples.ProjectionsTheory}{projections_theory}), the $ \lambda $-theory $ \Lambda $ (\coqident{AlgebraicTheories.Examples.LambdaCalculus}{lambda_calculus_lambda_theory}), the $ T $-presheaf structure on $ T $ and the $ T $-algebra structure on $ T_n $.
% FreeMonoidTheory.v FreeObjectTheory.v
  \item The `free object' algebraic theory $ T $ (\coqident{AlgebraicTheories.Examples.FreeObjectTheory}{free_object_theory}) with a functor from $ \C $ to $ \Alg T $ (\coqident{AlgebraicTheories.Examples.FreeObjectTheory}{free_object_algebra_functor}). Also, for the special case where $ \C $ is the category of monoids, a proof that this functor $ \C \to \Alg T $ is an equivalence (\coqident{AlgebraicTheories.Examples.FreeMonoidTheory}{monoid_algebra_equivalence}).
% EndomorphismTheory.v
  \item The construction of the endomorphism theory $ E(X) $ for some object $ X : \C $ in a category $ \C $ with finite products (\coqident{AlgebraicTheories.Examples.EndomorphismTheory}{endomorphism_lambda_theory}).
% Combinators.v CategoryOfRetracts.v OriginalRepresentationTheorem.v
  \item The original version of Scott's representation theorem (\coqident{AlgebraicTheories.OriginalRepresentationTheorem}{representation_theorem_iso}).
% Plus1Presheaf.v RepresentationTheorem.v
  \item Hyland's version of Scott's representation theorem (\coqident{AlgebraicTheories.RepresentationTheorem}{presheaf_lambda_theory_iso}), including the construction of the presheaf $ A(P, 1) $ (\coqident{AlgebraicTheories.Examples.Plus1Presheaf}{plus_1_presheaf}) and the proof that it is the exponential object $ P^T $ (\coqident{AlgebraicTheories.RepresentationTheorem}{theory_presheaf_exponentiable}).
% AlgebraToTheory.v
  \item The construction of a $ \lambda $-theory from a $ \Lambda $-algebra $ A $ as the endomorphism theory $ E_{\RAct{A_1}}(U_A) $, as discussed in Definition \ref{def:lambda-algebra-to-lambda-theory} (\coqident{AlgebraicTheories.FundamentalTheorem.AlgebraToTheory}{lambda_algebra_theory}).
% AlgebraicTheoryToLawvereTheory.v
  \item The construction of a Lawvere theory $ \cat L $ from an algebraic theory $ T $ (\coqident{AlgebraicTheories.AlgebraicTheoryToLawvereTheory}{algebraic_theory_to_lawvere}), and the equivalence between the presheaf category $ PL $ and the presheaf category $ \Pshf T $ (\coqident{AlgebraicTheories.AlgebraicTheoryToLawvereTheory}{algebraic_presheaf_weq_lawere_presheaf}).
% ExtensionsTheory.v
  \item The theory of extensions $ T_A $ of a $ T $-algebra (\coqident{AlgebraicTheories.Examples.ExtensionsTheory}{extensions_theory}), the equivalence between $ \Alg {T_A} $ and $ A \downarrow \Alg T $ (\coqident{AlgebraicTheories.Examples.ExtensionsTheory}{algebra_coslice_equivalence}) and the factorization of every theory morphism $ f: S \to T $ through $ S_{f^*(T_0)} $ (\coqident{AlgebraicTheories.Examples.ExtensionsTheory}{factorization}).
% LambdaCalculus.v
  \item An axiomatic definition of the pure $ \lambda $-calculus as discussed in Section \ref{sec:axiomatic-lambda-calculus} (\coqident{AlgebraicTheories.LambdaCalculus}{lambda_calculus}).
\end{itemize}
...
\begin{itemize}
  \item Univalence of the Sigma displayed category (\coqident{CategoryTheory.DisplayedCats.Examples.Sigma}{is_univalent_sigma_disp}).
  \item The Sigma displayed category creates limits (\coqident{CategoryTheory.DisplayedCats.Examples.Sigma}{creates_limits_sigma_disp_cat}).
  \item The definitions $ \overline \C $ (\coqident{AlgebraicTheories.FundamentalTheorem.CommonUtilities.KaroubiEnvelope}{karoubi_envelope}) and $ \widetilde \C $ (\coqident{AlgebraicTheories.FundamentalTheorem.CommonUtilities.KaroubiEnvelope}{karoubi_envelope'}) for the Karoubi envelope, together with some properties and their equivalence (\coqident{AlgebraicTheories.FundamentalTheorem.CommonUtilities.KaroubiEnvelope}{karoubi_equivalence}).
  \item The contents of Section \ref{sec:monoid-category}: The functorial construction of a one-object category from a monoid (\coqident{CategoryTheory.Categories.MonoidToCategory}{monoid_to_category}), the equivalence between it's presheaves and sets with a right monoid action (\coqident{CategoryTheory.Categories.MonoidToCategory}{monoid_presheaf_action_equivalence}), properties of the category of sets with a monoid action and restriction and extension of scalars (\coqident{AlgebraicTheories.FundamentalTheorem.CommonUtilities.MonoidActions}{scalar_restriction_functor}, \coqident{AlgebraicTheories.FundamentalTheorem.CommonUtilities.MonoidActions}{scalar_extension_functor}).
  \item The Yoneda embedding preserves exponential objects (\coqident{CategoryTheory.YonedaExponentials}{yoneda_preserves_exponentials}).
  \item The uniqueness of the Rezk completion (\coqident{Bicategories.RezkCompletions.Uniqueness}{rezk_completion_unique}).
% FundamentalTheorem/SurjectivePrecomposition.v
  \item If $ F : \C_1 \to \C_2 $ is a fully faithful functor, and $ \D $ is a category with colimits, then the precomposition functor $ F \bullet - : [\C_2, \D] \to [\C_1, \D] $ is split essentially surjective (\coqident{AlgebraicTheories.FundamentalTheorem.SurjectivePrecomposition}{pre_comp_split_essentially_surjective}).
  \item A proof of a generalized version of Lemma \ref{lem:equivalent-presheaf-cats}, showing that if $ F_1 : \C_1 \to \C_2 $ is a functor, if $ F_2 : \C_2 \to \C_3 $ is a fully faithful functor, if $ \D $ is a category with colimits and if the precomposition $ (F_1 \bullet F_2) \bullet - : [\C_3, \D] \to [\C_1, \D] $ is an adjoint equivalence, then $ F_1 \bullet - $ and $ F_2 \bullet - $ are adjoint equivalences too (\coqident{CategoryTheory.Equivalences.EquivalenceFromComp}{adjoint_equivalence_1_from_comp}).
% IndexedSetCategory.v
  \item The univalent category $ \SET^A $ of indexed sets $ (X_a)_a $ over a type $ A $ (\coqident{AlgebraicTheories.IndexedSetCategory}{indexed_set_cat}), which can be defined formally as
  \[ \SET^A_0 = (A \to \SET) \quad \text{and} \quad \SET^A(X, Y) = \prod_a X_a \to Y_a \]
  that is univalent (\coqident{AlgebraicTheories.IndexedSetCategory}{is_univalent_indexed_set_cat}) and has limits (\coqident{AlgebraicTheories.IndexedSetCategory}{limits_indexed_set_cat}).
\end{itemize}
...
\begin{center}
  \begin{tikzcd}[math mode=false]
    \coqident{AlgebraicTheories.AlgebraCategoryCore}{algebra_cat} \arrow[d]
    & \coqident{AlgebraicTheories.LambdaTheoryCategoryCore}{lambda_theory_cat} \arrow[d]
    & \coqident{AlgebraicTheories.PresheafCategoryCore}{presheaf_cat} \arrow[d]
    \\ \coqident{AlgebraicTheories.AlgebraCategoryCore}{algebra_data_full_cat} \arrow[d]
    & \coqident{AlgebraicTheories.LambdaTheoryCategoryCore}{lambda_theory_data_cat} \arrow[d]
    & \coqident{AlgebraicTheories.PresheafCategoryCore}{presheaf_data_cat} \arrow[d]
    \\ \begin{tabular}{c}\coqident{AlgebraicTheories.AlgebraicTheoryCategoryCore}{algebraic_theory_cat} \\ $ \times $ \coqident{CategoryTheory.Categories.HSET.Core}{hset_category} \end{tabular} \arrow[r]
    & \coqident{AlgebraicTheories.AlgebraicTheoryCategoryCore}{algebraic_theory_cat} \arrow[d]
    & \begin{tabular}{c}\coqident{AlgebraicTheories.AlgebraicTheoryCategoryCore}{algebraic_theory_cat} \\ $ \times $ \coqident{AlgebraicTheories.IndexedSetCategory}{indexed_set_cat} \end{tabular} \arrow[l]
    \\ & \coqident{AlgebraicTheories.AlgebraicTheoryCategoryCore}{algebraic_theory_data_cat} \arrow[d] &
    \\ & \coqident{AlgebraicTheories.IndexedSetCategory}{indexed_set_cat} &
  \end{tikzcd}
\end{center}
...
This is all very reminiscent of the way that displayed categories `borrow' information from their base category. Now if a category $ C $ has binary products, we say that a displayed category $ D $ over $ C $ \textit{creates binary products}, if for all $ \overline X : D_X $, $ \overline Y : D_Y $, we can `lift' their product. That is, if we can find
...
\begin{proof}
  ...
  \begin{center}
    \begin{tikzcd}
      \overline X_i & \overline Y \arrow[d, "\overline f" description] \arrow[l, "\overline p_i"] & \overline Z \arrow[ll, "\overline g_i"', bend right] \arrow[l, "\overline h"] \arrow[ld, "{\langle \overline g_1, \overline g_2 \rangle}"] \\
      & \overline X_1 \times \overline X_2 \arrow[lu, "\overline \pi_i"] &
    \end{tikzcd}
  \end{center}
...
\subsection{Propagation of Substitution}\label{sec:tactic}
