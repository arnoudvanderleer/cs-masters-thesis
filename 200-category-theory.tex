...
\begin{definition}
  A functor $ F: \C \to \D $ is called a \iindex{weak equivalence} if it is essentially surjective and fully faithful. We will sometimes denote a weak equivalence with
  \begin{center}
    \begin{tikzcd}
      \C \arrow[r, hook, two heads] & \D
    \end{tikzcd}
  \end{center}
...
\begin{definition}
  A morphism $ f: \E(Y, Z) $ is called \iindex{cartesian} if for all $ g: \E(X, Z) $ and $ h: \B(P(X), P(Y)) $ with $ h \cdot P(f) = P(g) $, there exists $ \overline h: \E(X, Y) $ such that $ P(\overline h) = h $ and $ \overline h \cdot f = g $, like illustrated in the following diagram from \autocite{nlab:grothendieck_fibration}
  \begin{center}
    \begin{tikzcd}[sep=large]
      \E \arrow[d, "P"] &X \arrow[rr, "\forall g", bend left] \arrow[r, "\exists! \overline h"', dashed] & Y \arrow[r, "f"'] & Z\\
...
\begin{definition}
  $ P $ is a \iindex{fibration} if for all $ Y: \E $ and morphisms $ f: \B(X, P(Y)) $, there exist an object $ \overline X: \E $ and a cartesian morphism $ \overline f: \E(\overline X, Y) $ such that $ P(\overline X) = X $ and $ P(\overline f) = f $:
  \begin{center}
    \begin{tikzcd}
      \E \arrow[d, "P"] & \overline X \arrow[r, "\exists \overline f", dashed] & Y\\
...
Now we turn our attention to dependent sums. In $ \SET $, let $ (Y_x)_x $ and $ (Y^\prime_x)_x $ be two families over $ X $ and let $ ((Z_y)_{y : Y_x})_{x : X} $ be a family of families. Let $ f: \sum_{x : X} Y_x \to X $ be the internal representation of $ (Y_x)_x $. A family of maps $ g_x : (\sum_{y : Y_x} Z_y)_x \to Y^\prime_x $ consists of maps $ Z_y \to Y^\prime_x $ for all $ y : Y_x $, so these are maps $ g_y : Z_y \to Y^\prime_{f(y)} $. This gives a bijection
...
\begin{definition}
  Precomposition gives a functor between functor categories $ F_* : [\B, \C] \to [\A, \C] $. If $ F_* $ has a left adjoint, we will call this adjoint functor the \textit{left Kan extension}\index{Kan extension!left} along $ F $ and denote it $ \mathrm{Lan}_F : [\A, \C] \to [\B, \C] $.
...
\begin{proof}
  ...
  which exactly describes the morphisms in $ \overline C $.
  \begin{center}
    \begin{tikzcd}
...
\begin{remark}\label{rem:classical-karoubi-universal}
  ...
  \begin{itemize}
    \item In $ \overline \C $, every idempotent splits.
    \item We have a fully faithful embedding $ \iota_\C : \C \to \overline \C $;
    \item Every object in $ \overline \C $ is a retract of an object in $ \C $.
  \end{itemize}
  ...
  There is another construction that yields a category with the properties listed above. An object in this alternative category is a presheaf $ F : P\C $ that is a retract of the Yoneda embedding $ \yo(X) $ of an object $ X : \C $. Because it satisfies the universal property, it is classically equivalent to $ \overline \C $.
\end{remark}
...
There are two different ways to translate the definition at the end of Remark \ref{rem:classical-karoubi-universal} to univalent foundations. We can either interpret the existence of the object $ X $ and the retraction $ (r, s) : F \triangleleft \yo(X) $ as additional \textit{structure} on $ F $, or we can treat it as a \textit{property} and ask for \textit{mere existence} (see Definition \ref{def:mere-existence}) of $ X $, $ r $ and $ s $. This gives rise to two different categories:

\begin{definition}[\coqident{AlgebraicTheories.FundamentalTheorem.CommonUtilities.KaroubiEnvelope}{karoubi_envelope'}]\label{def:karoubi'}
  We define the category $ \widetilde \C $ in which every object is a presheaf $ F : P \C $, together with an object $ X : \C $ and a retraction-section pair $ (r, s) : F \triangleleft \yo (X) $. The morphisms from $ (F_1, X_1, r_1, s_1) $ to $ (F_2, X_2, r_2, s_2) $ are just the presheaf morphisms $ f: P\C(F_1, F_2) $. This can be summarized in the following diagram:
  \begin{center}
    \begin{tikzcd}
...
\begin{definition}\label{def:karoubi''}
  We define the category $ \widehat \C $ as the full subcategory of $ P \C $ consisting of objects $ F : P \C $ such that there merely exist an object $ X: \C $ and a retraction-section pair $ (r, s) : F \triangleleft \yo(X) $, summarized in the following diagram:
  \begin{center}
    \begin{tikzcd}
  ...
  \end{center}
\end{definition}
Because it is a full subcategory of a presheaf category (which is univalent), $ \widehat \C $ is univalent.

Now classically, $ \overline \C $ is equivalent to $ \widetilde \C $ or $ \widehat \C $. In general, this does not work for $ \widehat \C $, but it does work for $ \widetilde \C $:
\begin{lemma}[\coqident{AlgebraicTheories.FundamentalTheorem.CommonUtilities.KaroubiEnvelope}{karoubi_equivalence}]\label{lem:karoubi-equivalence}
  We have an adjoint equivalence $ \overline \C \xrightarrow \sim \widetilde \C $.
\end{lemma}
\begin{proof}
  ...
  Note that this is an equivalence on the morphisms: For any morphism $ \overline g : P\C(F_1, F_2) $, we have
  \[ r_1 \cdot \overline g \cdot s_2 : P\C(\yo(X_1), \yo(X_2)). \]
  ...
  \begin{center}
    \begin{tikzcd}
      F_1 \arrow[r, shift left, "s_1"] \arrow[d, "\overline g"] & \yo(X_1) \arrow[l, shift left, "r_1"] \arrow[r, shift left, "\id{\yo(X_1)}"] \arrow[r, shift right, "\yo(f_1)"'] \arrow[d, "\yo(g)"] & \yo(X_1)\\
      F_2 \arrow[r, shift left, "s_2"] & \yo(X_2) \arrow[l, shift left, "r_2"] \arrow[r, shift left, "\id{\yo(X_2)}"] \arrow[r, shift right, "\yo(f_2)"'] & \yo(X_2)\\
    \end{tikzcd}
  \end{center}

  Now, note that this map is also split essentially surjective. For some $ (F, X, r, s) : \widetilde \C $, $ r \cdot s $ is an idempotent morphism on $ \yo(X) $, and by fully faithfulness of the Yoneda Lemma, it corresponds to an idempotent morphism $ f $ on $ X $. We send $ (F, X, r, s) $ to $ (X, f) $. Note that both $ F $ and the image of $ (X, f) $ are equalizers of $ \id{\yo(X)} $ and $ r \cdot s $, so they are isomorphic.

  The fact that a fully faithful and split essentially surjective functor is an adjoint equivalence concludes the proof.
\end{proof}

\begin{remark}
  Note that since the morphism types of $ \widetilde \C $ and $ \widehat \C $ are the same, and since the objects of $ \widehat \C $ are just truncated versions of the objects in $ \widetilde \C $, we have a fully faithful embedding
  \[ \widetilde \C \hookrightarrow \widehat \C, \]
  which just forgets the choices for $ X $, $ r $ and $ s $ on the objects. Note that this is also essentially surjective: by definition, for any $ (F, H) : \widehat \C $, there merely exist $ X $, $ r $ and $ s $ such that $ (X, r, s) $ truncates to $ H $.
  Therefore, we have a weak equivalence from $ \widetilde \C $ to $ \widehat \C $. However, as we will see, $ \widetilde \C $ is usually not univalent, so this does not give an adjoint equivalence of categories.
\end{remark}
...
\begin{corollary}\label{cor:karoubi-candidates}
  ...
  \begin{center}
    \begin{tikzcd}
      \C \arrow[r, hook, "\iota_\C"] & \overline \C \arrow[r, "\sim"] & \widetilde \C \arrow[r, hook, two heads] & \widehat \C \arrow[r, phantom, "\subseteq"] & P\C
    \end{tikzcd}
  \end{center}
  The fact that $ \widehat \C $ is univalent, and has weak equivalences from $ \overline \C $ and $ \widetilde \C $ exhibits $ \widehat \C $ as the Rezk completion of $ \overline \C $ and $ \widetilde \C $.
\end{corollary}
...
\begin{lemma}[\coqident{AlgebraicTheories.FundamentalTheorem.CommonUtilities.KaroubiEnvelope}{karoubi_univalence}]\label{rem:karoubi-univalent}
...
\begin{remark}\label{rem:karoubi-not-univalent}
...
We will also briefly study the identity types of $ \widetilde \C $.
\begin{lemma}\label{lem:isaset-karoubi'}
  If $ \C $ is a setcategory, $ \widetilde \C $ is a setcategory as well.
\end{lemma}
\begin{proof}
  Take $ P = (F_1, X_1, r_1, s_1) $ and $ Q = (F_2, X_2, r_2, s_2) $ as objects of $ \widetilde \C $. The identity type $ P = Q $ is equivalent to
  ...
  \begin{center}
    \begin{tikzcd}
      F_1 \arrow[d, "\mathrm{idtoiso}(f)"'] \arrow[r, "s_1", shift left] & \yo(X_1) \arrow[d, "\yo(\mathrm{idtoiso}(g))"] \arrow[l, "r_1", shift left] \\
      F_2 \arrow[r, "s_2", shift left] & \yo(X_2) \arrow[l, "r_2", shift left]
    \end{tikzcd}
  \end{center}
  Since $ P \C $ is univalent,
  \[ f = \mathrm{isotoid}(\mathrm{idtoiso}(f)) = \mathrm{isotoid}(s_1 \cdot \yo(\mathrm{idtoiso}(g)) \cdot r_2). \]
  Since morphisms in $ \C $ form a set, the identities on the $ r_i $ and $ s_i $ are mere propositions, and the only `structure' of the identity type is the equality $ X_1 = X_2 $. Therefore, if $ \C $ is a setcategory, $ \widetilde \C $ is a setcategory as well.
\end{proof}
Since $ \widetilde \C $ has the same isomorphisms as $ \overline \C $, $ \widetilde \C_M $ is not univalent for $ M $ as in Remark \ref{rem:karoubi-not-univalent}.

\begin{remark}\label{rem:karoubi-candidate-choice}
  As mentioned in Remark \ref{rem:classical-karoubi-universal}, in univalent foundations, there are two different Karoubi envelopes: one that is a setcategory and one that is univalent. As we have seen above, if $ \C $ is a setcategory, both $ \overline \C $ and $ \widetilde \C $ are setcategories, so they are the Karoubi envelopes in the theory of setcategories. Also, since $ \widehat \C $ is univalent, it is the Karoubi envelope in the theory of univalent categories.

  As mentioned before, in univalent foundations $ \widehat \C $ is usually not equivalent to $ \overline \C $ and $ \widetilde \C $ (Corollary \ref{cor:karoubi-candidates}), because they are the answer to different questions. In practice, we need to be careful which definition we choose, because this choice has consequences.

  On one hand, $ \widehat \C $ is univalent, but very abstract. In this category, there are classical constructions that we cannot do, because for every object, we only have mere existence of an idempotent morphism, and because the objects of $ \widehat \C $ do not form a set, we cannot use the axiom of choice to pick an idempotent morphism.

  On the other hand, $ \overline \C $ and $ \widetilde \C $ are very elementary and concrete, which sometimes helps when doing constructions. However, they are not univalent, which makes working with them complicated in a different way.
\end{remark}
...
For convenience, the lemma below works very abstractly with pointwise left Kan extension using colimits, but one could also prove this using just (co)equalizers (or right Kan extension using $ \C $-small limits).
