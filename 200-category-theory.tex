...
There is also the dual concept of a universal arrow $ (X, f) $ from a functor $ F $ to an object $ Y: C $. Its universal property can be summarized in the following diagram:
\begin{center}
  \begin{tikzcd}
...
For a functor between categories $ f: C \to D $, given the Yoneda embeddings $ \yo_C : C \to P C $ and $ \yo_D : D \to P D $ (we will often omit the subscript $ C $ and $ D $), we can create a diagram
\begin{center}
  \begin{tikzcd}
...
\begin{proof}
  ...
  \begin{center}
    \begin{tikzcd}
      Z_1 \arrow[rdd, "g_1"'] \arrow[rd, "\alpha_1"] \arrow[rr, "\beta"] && Z_2 \arrow[ldd, "g_2"] \arrow[ld, "\alpha_2"']\\
      & Y \arrow[d, "f" description]\\
      & X
    \end{tikzcd}
    $ \Leftrightarrow $
    \begin{tikzcd}
      Z_1 \arrow[rd, "\alpha_1"] \arrow[rr, "\beta"] && Z_2 \arrow[ld, "\alpha_2"']\\
      & Y
    \end{tikzcd}
  \end{center}
...
\begin{proof}
  ...
  \begin{center}
    \begin{tikzcd}
      Z \arrow[rd, "\beta_1"] \arrow[rr, "\beta_2", bend left] \arrow[rrd, "\alpha"', bend right=49, shift right] \arrow[r, dashed, "\gamma"] & Y_1 \times_X Y_2 \arrow[r, "h_2"'] \arrow[d, "h_1"] \arrow[rd, dashed, "g"] & Y_2 \arrow[d, "f_2"] \\
      & Y_1 \arrow[r, "f_1"] & X
    \end{tikzcd}
  \end{center}
\end{proof}
...
\[ (\SET \downarrow X)\left(f^*(\{ \star \}, \id{\{\star\}}), \left(\sum_{x : X} Y_x, \pi_1 \right) \right) \simeq (\SET \downarrow {\{\star\}})\left( (\{\star\}, \id{\{ \star \}}), \left( \prod_{x : X} Y_x, ! \right) \right) \]
...
\[ \left(\SET \downarrow \sum_{x : X} Y_x \right)\left(f^*(X, \id X), (\sum_{x : X} \sum_{y : Y_x} Z_y, p) \right) \simeq (\SET \downarrow A)\left((X, \id X), \left(\sum_{x : X} \prod_{y : Y_x} Z_y, \pi_1\right)\right). \]
...
\begin{definition}
  ...
  \begin{center}
    \begin{tikzcd}
      (C \downarrow A) \arrow[r, shift left=2, "f^*"{name=A}] &
      (C \downarrow B) \arrow[l, shift left=2, "\prod_f"{name=B}]
...
\begin{remark}\label{rem:retract-coequalizer}
  ...
  Then $ d^\prime $ is the equalizer of \begin{tikzcd}
    d \arrow[r, shift left, "\id d"] \arrow[r, shift right, "r \cdot s"'] & d \end{tikzcd} in the following way:
