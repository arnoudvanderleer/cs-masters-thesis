...
We will also briefly study the identity types of $ \tilde \C $.
\begin{lemma}\label{lem:isaset-karoubi'}
  If $ \C $ is a setcategory, $ \tilde \C $ is a setcategory as well.
\end{lemma}
\begin{proof}
  Take $ P = (F_1, X_1, r_1, s_1) $ and $ Q = (F_2, X_2, r_2, s_2) $ as objects of $ \tilde \C $. The identity type $ P = Q $ is equivalent to
...
\end{proof}
Since $ \tilde \C $ has the same isomorphisms as $ \bar \C $, $ \tilde \C_M $ is not univalent for $ M $ as in Remark \ref{rem:karoubi-not-univalent}.
