...
\begin{remark}\label{rem:karoubi-classical}
  The situation in classical mathematics is very similar to the situation above: We can define a category $ \bar \D $ which is defined using idempotents, a category $ \tilde \D $ which consists of presheaves with chosen retractions, and a category $ \hat \D $ which is the image of $ \tilde \D $ under the forgetful functor $ F : \hat \D \to P \C $. To be more precise, $ \hat \D $ is the full subcategory of $ P \C $, which we get if we only select the presheaves $ F $ such that we can find some $ X : \C $ and some retraction $ (r, s) : F \triangleleft \yo(X) $. We again have an adjoint equivalence $ \bar \D \simeq \tilde \D $ and a fully faithful, essentially surjective functor $ F : \tilde \D \to \hat \D $.
