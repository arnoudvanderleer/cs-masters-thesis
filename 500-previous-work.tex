...
\begin{itemize}
  \item Types in the $ \lambda $-calculus correspond to objects in the category.
  \item Types $ A \to B $ in the $ \lambda $-calculus correspond to exponential objects $ B^A $ in the category.
  \item Terms in the $ \lambda $-calculus of type $ B $, with free variables $ x_1: A_1, \dots, x_n: A_n $, correspond to morphisms $ A_1 \times \dots \times A_n \to B $.
  \item A free variable $ x_i: A_i $ in a context with free variables $ x_1: A_1, \dots, x_n: A_n $ corresponds to the projection morphism $ \pi_i : A_1 \times \dots \times A_n \to A_i $.
  \item Given a term $ s: B_1 \to B_2 $ and a term $ t: B_1 $, both with free variables $ x_1: A_1, \dots, x_n: A_n $, corresponding to morphisms $ \overline s: A_1 \times \dots \times A_n \to B_2^{B_1} $ and $ \overline t: A_1 \times \dots \times A_n \to B_1 $, the application $ st: B_2 $ corresponds to the composite of the product morphism with the evaluation morphism $ A_1 \times \dots \times A_n \to B_2^{B_1} \times B_1 \to B_2 $.
    \begin{center}
      \begin{tikzcd}
      ...
      \end{tikzcd}
    \end{center}
  \item Given a term $ t: B $ with free variables $ x_1: A_1, \dots, x_n: A_n $, the abstraction $ (\lambda x_n, t): A_n \to B $ corresponds to using the adjunction corresponding to the exponential object of $ A_n $:
    \[ \C(A_1 \times \dots \times A_{n-1} \times A_n, B) \cong \C(A_1 \times \dots \times A_{n-1}, B^{A_n}). \]
\end{itemize}
...
\begin{definition}[\coqident{AlgebraicTheories.CategoryOfRetracts}{R}]
  Note that the set of $ \lambda $-terms without free variables $ L_0 $ has a monoid structure under the composition defined above. The category \index{R}$ \R $ is defined as the Karoubi envelope $ \overline C_{L_0} $ (Definition \ref{def:karoubi}) of the category $ C_{L_0} $ of this monoid.
...
\begin{remark}
  As we saw in Remarks \ref{rem:karoubi-univalent} and \ref{rem:karoubi-not-univalent}, the Karoubi envelope can only be univalent if the original category is univalent, and even then it is not always univalent. In the case of $ \R $, if $ L $ is a nontrivial $ \lambda $-theory, $ \R $ is not a univalent category. To see this, note, for example, that we have an object $ X := \langle \pi_1, \pi_2 \rangle : \R $ (corresponding to the type of `pairs' of $ \lambda $-terms). Since $ L_0 $ is a set, $ X = X $ is a proposition. However, $ X $ has (at least) two automorphisms:
...
\begin{remark}
  In some cases, we can already conclude that $ \R $ is not univalent because $ L_0 $ is not univalent. For example, if $ L_0 $ has $ \eta $-equality, the single object $ \star : L_0 $ has a nontrivial automorphism given by
  \[ \lambda x_1 x_2 x_3, x_1 x_3 x_2, \]
  so $ L_0 $ is not univalent, so $ \R $ is not univalent.

  On the other hand, if $ L $ is the pure $ \lambda $-calculus with $ \beta $-equality, I believe we can use structural induction to show that $ \star : L_0 $ has no nontrivial automorphisms, so $ L_0 $ is univalent. However, as shown above, $ \R $ is still not univalent.
\end{remark}
...
\[ \R(A^\prime, A) \cong \R(I \times A^\prime, A) \cong \R(I, A^{A^\prime}) \cong \{ \star \}, \]
...
\begin{proof}
  ...
  We have $ U^U = \lambda f, U \circ f \circ U = \lambda x_1 x_2, x_1 x_2 $. Using the bijection $ \R(U^n \times U, U) \cong \R(U^n, U^U) $ and the retraction $ U^U: U \to U^U $, the abstraction and application $ \lambda $ and $ \rho $ are given by
...
\begin{enumerate}
  ...
  \item Given display maps $ f: B \to A $ and $ g: C \to B $. We have $ X : \R^A $ and $ Y: \R^B $ with isomorphisms $ s: B \xrightarrow \sim \sum_A X $ and $ t: C \xrightarrow \sim \sum_B Y $. We need to show that $ f \circ g: C \to A $ is also a display map.
    \begin{center}
      ...
      \end{tikzcd}
    \end{center}
    Intuitively, the isomorphism $ \sum_B Y \cong \sum_A Z $ represents a bijection
    \[ \sum_{b: \sum_{a: A} X a} Y b \cong \sum_{a: A} \sum_{x: X a} Y (a, x), \]
    relating $ ((a, x), y) $ to $ (a, (x, y)) $. Therefore, consider
...
\begin{proof}
  ...
  \begin{center}
    \begin{tikzcd}
      Z \circ \alpha \arrow[rd, "f"'] \arrow[r, "{\widehat f \circ \alpha}", dashed] & (\alpha_* Y) \circ \alpha \arrow[d, "{\epsilon_Y}"]\\
      & Y
    \end{tikzcd}
  \end{center}
  Now, suppose that we have some $ Z : \R^A $ and some $ f: \R^B(Z \circ \alpha, Y) $. We define
  \[ \widehat f = \lambda x_1 x_2 x_3, f(g^{-1}(x_1, x_3)) (Z x_1 x_2): \R^A(Z, \alpha_* Y) \]
  and we have $ ((\widehat f) \circ \alpha) \cdot \epsilon_Y = f $.
  % \begin{align*}
  %   (((\alpha \cdot -) \widehat f) \cdot \epsilon_Y) b z
  %   &= \epsilon_Y b ((\widehat f (\alpha b)) z)\\
  ...
  %   &= f b z.
  % \end{align*}
  Now, suppose that we have some (other) $ \widehat f^\prime : \R^A(Z, \alpha_*(Y)) $ such that $ (\widehat f^\prime \circ \alpha) \cdot \epsilon_Y = f $. Then substituting $ (\widehat f^\prime \circ \alpha) \cdot \epsilon_Y $ for $ f $ in $ \widehat f $ yields
  \[ \widehat f = \lambda x_1, (\alpha_* Y x_1) \circ (\widehat f^\prime x_1) = \widehat f^\prime, \]
  so $ \widehat f $ is unique and $ (\alpha_* Y, \epsilon_Y) $ is a universal arrow, which concludes the proof that $ (\alpha \cdot -) \dashv \alpha_* $ and we see that $ \R $ is cartesian closed relative to the given class of display maps.
  % \begin{align*}
  %   \widehat f
  %   &= \lambda x_1 x_2 x_3, f (g^{-1} (x_1, x_3))(Z x_1 x_2)\\
  %   &= \lambda x_1 x_2 x_3, ((\epsilon_Y (g^{-1} (x_1, x_3))) \circ ((\widehat f^\prime \circ \alpha) (g^{-1} (x_1, x_3)))) (Z x_1 x_2)\\
  %   &= \lambda x_1 x_2 x_3, (\epsilon_Y (g^{-1} (x_1, x_3))) (\widehat f^\prime (\alpha (g^{-1} (x_1, x_3))) (Z x_1 x_2))\\
  %   &= \lambda x_1 x_2 x_3, (\epsilon_Y (g^{-1} (x_1, x_3))) (\widehat f^\prime x_1 x_2)\\
  %   &= \lambda x_1 x_2 x_3, Y (g^{-1} (x_1, x_3)) (\widehat f^\prime x_1 x_2 (\pi_2 (g (g^{-1} (x_1, x_3)))))\\
  %   &= \lambda x_1 x_2 x_3, Y (g^{-1} (x_1, x_3)) (\widehat f^\prime x_1 x_2 (X x_1 x_3))\\
  %   &= \lambda x_1 x_2, \alpha_* Y x_1 (\widehat f^\prime x_1 (Z x_1 x_2))\\
  %   &= \widehat f^\prime
  % \end{align*}
\end{proof}
...
Now, it is not hard to show that Hyland's and Selinger's notions of $ \lambda $-theory are equivalent: If we have one of Selinger's $ \lambda $-theories with a countable set of variables, enumerate the variables as $ x_1, x_2, \dots $ and let $ L_n $ be the set of terms with only the free variables $ x_1, \dots, x_n $. Then, if we quotient by the equivalence relation, we get one of Hyland's $ \lambda $-theories. Conversely, if we start with one of Hyland's $ \lambda $-theories $ L $, we can consider $ L_n $ as a subset of $ L_{n + 1} $ by sending $ x_{n, i} $ to $ x_{n + 1, i} $. If we take the union $ \overline L = \bigcup_n L_n $ (officially, this is a directed limit), we have a function $ f: \Lambda_{\overline L} \to \overline L $, because the constants of $ \Lambda_{\overline L} $ are exactly the terms of $ \overline L $ and because $ \lambda $-theories in Hyland's sense support the operations of the $ \lambda $-calculus (see Subsection \ref{subsec:lambda-calculus-operations}). This gives an equivalence relation on $ \Lambda_{\overline L} $ by taking $ a\ `='\ b $ if $ f(a) = f(b) $.
