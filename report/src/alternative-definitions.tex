...
\begin{definition}
  An \iindex{algebraic theory} as presented in \textcite{algebaric-theories-2010} is a small category with finite products.
\end{definition}
\begin{definition}
...
\begin{lemma}\label{lem:lawvere-clone}
...
\begin{lemma}\label{lem:lawvere-presheaf}
...
\begin{definition}
  ...
  written $ (f, g_1, \dots, g_m) \mapsto f[g_1, \dots, g_m] $, satisfying some identity, associativity and naturality conditions (see \textcite{lambda-monoid}, Definition 2.1, for more details).
\end{definition}
...
\begin{remark}
  Note that for an adjunction $ F \dashv G $, $ F \bullet G $ gives a monad. In the same way, there exists a notion of \iindex{relative adjunction}, from which we can obtain a relative monad (see \textcite{monads-endofunctors}, Theorem 2.10).
\end{remark}
...
For details and a general treatment, see \textcite{monads-endofunctors}, Section 3 and specifically Theorem 3.4.
...
\begin{definition}
  Given a (skew-)monoidal category $ (C, \otimes, I) $, a \iindex{monoid} in this category is an object $ T : C $, together with a `multiplication' $ \mu: C(T \otimes T, T) $ and a `unit' morphism $ \eta: C(I, T) $ satisfying a couple of laws (see \textcite{MacLane}, Section III.6).
\end{definition}
