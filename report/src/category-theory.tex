...
I will assume a familiarity with the category-theoretical concepts presented in \textcite{CT4P}. These include categories, functors, isomorphisms, natural transformations, adjunctions, equivalences and limits.
...
One concept in category theory that can be used to describe a lot of limits and adjunctions is that of a universal arrow (see for example \textcite{MacLane}, Part III)
\begin{definition}
...
An alternative characterization (see \textcite{MacLane}, chapter IV.1, Theorem 2) of an adjunction $ L \dashv R $ is as a natural bijection
...
\end{lemma}
\begin{proof}
  See \textcite{MacLane}, Chapter IV.1, Theorem 2 (iv).
\end{proof}
...
\section{Yoneda}
We can embed a category $ C $ fully faithfully into the functor category $ P C = [\op C, \SET] $ as follows (see \textcite{Kashiwara}, Section 1.4):
\begin{definition}
...
\end{lemma}
\begin{proof}
  See \textcite{Kashiwara}, Proposition 1.4.3.
\end{proof}
...
\end{lemma}
\begin{proof}
  See \textcite{borceux}, Volume 1, Proposition 2.15.5.
\end{proof}

\begin{lemma}
  The Yoneda embedding functor preserves exponential objects (\textcite{stackexchange:yoneda-exponentials}).
\end{lemma}
\begin{proof}
...
\begin{definition}
  A morphism $ f: E(y, z) $ is called \iindex{cartesian} if for all $ g: E(x, z) $ and $ h: B(P(x), P(y)) $ with $ h \cdot P(f) = P(g) $, there exists $ \bar h: E(x, y) $ such that $ P(\bar h) = h $ and $ \bar h \cdot f = g $, like illustrated in the following diagram from \textcite{nlab:grothendieck_fibration}
  \begin{center}
...
\end{lemma}
\begin{proof}
  See \textcite{borceux}, Volume 3, Lemma 5.8.2. This lemma shows this via the following equivalences:
  \begin{align*}
...
The following is based loosely on Section 4.1 of \textcite{taylor}.
...
\end{lemma}
\begin{proof}
  See the end of Section 1.3 of \textcite{freyd}.
\end{proof}
...
\begin{proof}
  ...
  By Proposition 1.34 in \textcite{freyd}, $ \prod_{p_1} \Delta_{X \times Z} W $ is given as the following pullback:
  \begin{center}
    \begin{tikzcd}
...
One of the most general and abstract concepts in category theory is the concept of \textit{Kan extensions}. In \textcite{MacLane}, Section X.7, MacLane notes that
\begin{quote}
...
If a category has limits (resp. colimits), we can construct the right (resp. left) Kan extension in a `pointwise' fashion (see Theorem X.3.1 in \textcite{MacLane} or Theorem 2.3.3 in \textcite{Kashiwara}). Below, I will outline the parts of the construction that we will need explicitly in this thesis.
\begin{lemma}
...
\end{lemma}
\begin{proof}
  To show that $ \eta $ is a natural isomorphism, we have to show that $ \eta_G(a^\prime): G(a^\prime) \Rightarrow \Lan F G(F(a^\prime)) $ is an isomorphism for all $ G: [A, C] $ and $ a^\prime: A $. Since a left adjoint is unique up to natural isomorphism (\textcite{CT4P}, Exercise 153), we can assume that $ \Lan F G(F(a^\prime)) $ is given by
...
\end{lemma}
\begin{proof}
  First of all, note that $ \hat C $ is univalent, since it is a presheaf category (see Theorem 4.5 in \textcite{univalent-categories}). So we must show that we have a weak equivalence from $ \overline C $ to $ \hat C $.
