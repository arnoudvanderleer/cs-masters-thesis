...
Let $ \llbracket n \rrbracket $ denote the finite set $ \{ 1, 2, \dots, n \} $. Let \index{$ \F $}$ \F \subseteq \FinSET $ be the full subcategory with $ \F_0 = \{ \llbracket 0 \rrbracket, \llbracket 1 \rrbracket, \dots \} $.
...
\begin{proof}
  ...
  \[ A + B = \int^{(m, n) : \F \times \F} A^m \times B^n \times T_{m^\prime + n^\prime}, \]
  considering $ T $ as a functor $ T : \F \to \SET $ as a functor (see Remark \ref{rem:abstract-clone-functor}). Note that we do not need the exact definition of $ A + B $ for the rest of this section. Nonetheless, it is interesting to see how it is defined and why this definition works.
  ...
  The relations then give `associativity' of this substitution and also assure that reordering or duplicating $ x_i $ and their corresponding $ a_i $ and $ b_j $ do not yield different elements: associating the images on the left and right of
  \[ A^{m^\prime} \times B^{n^\prime} \times T_{m^\prime + n^\prime} \leftarrow A^{m^\prime} \times B^{n^\prime} \times T_{m + n} \rightarrow A^m \times B^n \times T_{m + n} \]
  gives
  \[ (t \bullet ((x_{f(i)})_i + (x_{m + g(j)})_j)) \bullet a + b = t \bullet ((a_{f(i)})_i + (b_{g(j)})_j) \]
  for $ f: \llbracket m \rrbracket \to \llbracket m^\prime \rrbracket $, $ g: \llbracket n \rrbracket \to \llbracket n^\prime \rrbracket $, $ t : T_{m + n} $, $ a : A^{m^\prime} $ and $ b : B^{n^\prime} $.

  Then it becomes clear what the action of $ T $ will be on $ A + B $, although the precise definition has quite a combinatorial nature. For $ s : T_l $, $ t_i : T_{m_i + n_i} $, $ a_i : A^{m_i} $ and $ b_i : B^{n_i} $, define the disjoint embeddings
...
\begin{definition}[Theory of extensions]
  ...
  Now, take $ h: (T_n + A)^m $. Sending $ g: T_m $ to $ g \bullet h $ gives a $ T $-algebra morphism $ T_m \to T_n + A $. Together with the right injection morphism of $ A $ into $ T_n + A $, this gives us a $ T $-algebra morphism from the coproduct: $ T_m + A \to T_n + A $. In particular, we have a function on sets $ (T_m + A) \times (T_n + A)^m \to T_n + A $, which we will define our substitution to be.
\end{definition}
