...
Now, officially, $ \hat C_{L_0} $ is the full subcategory of $ P C_{L_0} $, consisting of all objects $ X : P C_{L_0} $ such that there merely exists $ (r, s) : X \triangleleft \yo(\star) $. However, recall from the previous section that $ P C_{L_0} \cong P C_{L_1} \cong \Pshf L $. Under this equivalence, $ \yo(\star) $ is embedded as the theory presheaf $ L $, and we will work in the full subcategory $ \hat \R \subseteq \Pshf L $ of the objects $ X : \Pshf L $ such that there merely exists $ (r, s) : X \triangleleft L $. Of course, then $ \hat R $ is equivalent to $ \hat C_{L_0} $.
...
\begin{enumerate}
  \item Take $ f : D(Y, X) $ and $ g: \hat \R(Z, X) $. We can borrow the pullback from $ \Pshf L $ to get $ Y \times_X Z : \Pshf L $.  We now must show that we have $ (r^\prime, s^\prime) : Y \times_X Z \triangleleft L $, and in this part, we can pretend that all the `merely exists' properties actually give us concrete elements.

    Now, note that taking pullbacks gives a functor $ g^* : (\Pshf L \downarrow X) \to (\Pshf L \downarrow Z) $. Because $ Y $ is a retract of $ X \times L $, $ Y \times_X Z $ is a retract of $ X \times L \times_X Z $, which is isomorphic to $ Z \times L $. Since $ Z $ is a retract of $ L $ (and since $ - \times L $ is a functor), $ Z \times L $ is a retract of $ L \times L $. But this is (isomorphic to) the image of $ U \times U : \R $, so this is is a retract of $ L $, and we can compose this to a retraction
    \[ Y \times_X Z \triangleleft Z \times L \triangleleft L \times L \triangleleft L. \]
    Because $ \hat \R $ is a full subcategory of $ \Pshf L $ and $ Y \times_X Z $ is the pullback of $ f $ and $ g $ in $ \Pshf L $, we have that $ Y \times_X Z $ is also the pullback of $ f $ and $ g $ in $ \hat \R $.

  \item Take $ f: D(Z, Y) $ and $ g: D(Y, X) $.
    \TODO : Composite

  \item Note that $ \Pshf L $ has a terminal presheaf $ I_n = \{ \star \} $. Under the embedding of $ \R $ into $ \Pshf L $, this is (isomorphic to) the image of $ \lambda x_1 x_2, x_2 : \R $, and therefore, it is a retract of $ L $. Now, given $ Y : \hat \R $, note that any morphisms between $ Y $ and $ I \times L $ trivially commute with the terminal projections. Since $ Y : \hat \R $, there merely exists $ (r, s) : Y \triangleleft L $, and since $ L \cong I \times L $, this means that the terminal projection is indeed in $ D(Y, I) $:
    \begin{center}
      \begin{tikzcd}
        Y \arrow[rd, "!"'] \arrow[rr, dashed, shift left, "s"] & & I \times L \arrow[ld, "!"] \arrow[ll, dashed, shift left, "r"]\\
        & I
      \end{tikzcd}
    \end{center}
\end{enumerate}
