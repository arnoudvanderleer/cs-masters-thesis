...
\end{lemma}
\begin{proof}
  Take $ B : \Alg{\Lambda_A} $. By Lemma \ref{lem:extensions-algebra-coslice} we can consider $ B $ to be a object in the coslice category $ h : \Alg{\Lambda}(A, B) $. As shown in Section \ref{sec:elementary-fundamental-theorem}, $ h $ sends elements in $ \R_A(U^n, U) $ to elements in $ \R_B(U^n, U) $, so we can regard it as a morphism $ h: \AlgTh(E_{\R_A}(U), E_{\R_B}(U)) $. We then have a $ E_{\R_A}(U) $-algebra $ h^*(E_{\R_B}(U)_0) $. Now we need to prove that we have an isomorphism of $ \Lambda_A $-algebras
  ...
  \begin{center}
    \begin{tikzcd}
      ...
      E_{\R_A}(U) \arrow[rr, "h"] && E_{\R_B}(U)
    \end{tikzcd}
  \end{center}
  Therefore, $ h_n(\iota_{\Lambda_n}(f)) = \iota_{\Lambda_n}(f) $, so $ B^\prime = \iota_\Lambda^*(E_{\R_B}(U)_0) $. We have the following diagram in $ \Alg \Lambda $:
  \begin{center}
    \begin{tikzcd}[sep = large]
      & A \arrow[ld, "a \mapsto h(\epsilon_A^{-1}(a))"'] \arrow[rd, "h"]\\
      E_{\R_B}(U)_0 \arrow[rr, "\epsilon_B"] & & B
    \end{tikzcd}
  \end{center}
  \TODO: Show commutativity

  which shows that $ \epsilon_B $ is an isomorphism in the coslice category under $ A $. Pulling this isomorphism back along the equivalence from Lemma \ref{lem:extensions-algebra-coslice} gives an isomorphism of $ \Lambda_A $-algebras
  \[ ([a \mapsto \epsilon_A^{-1}(a) \bullet (), \iota_{\Lambda_n}]_n \cdot h)^*(E_{\R_B}(U)_0) \cong B \]
  and this concludes the proof.
\end{proof}
...
\begin{lemma}
  ...
  and we will call the latter $ \langle \iota_\Lambda(s_i) \rangle_i $.
\end{lemma}
\begin{proof}
  By the recursive nature of the definitions of $ (s_i)_i $ and $ \langle a_i \rangle_i $, it suffices to show that for $ a, b : \Lambda_n $, $ \iota_\Lambda((a, b)) = \langle \iota_\Lambda(a), \iota_\Lambda(b) \rangle $ and that $ \iota_\Lambda(c_n) = I_c $. Since $ \iota_\Lambda $ is defined via structural induction, this is just a matter of straightforward but tedious unfolding and rewriting, at some point using the fact that $ \iota_\Lambda(a) \circ U^n = \iota_\Lambda(a) $:
  \begin{align*}
    \iota_\Lambda((a, b))
  ...
  \begin{align*}
    ...
    &= \lambda x_1 x_2, \pi_{n + 1, n + 1} (x_1, x_2)\\
    &= \lambda x_1 x_2, x_2\\
    &= I_c.
  \end{align*}
\end{proof}
...
\begin{remark}\label{rem:endomorphism-theory-algebra-to-endomorphism-theory-algebra}
  ...
  Functoriality of the endomorphism theory construction of the respective categories of retracts, gives a $ \lambda $-theory morphism
...
\begin{proof}
  ...
  \begin{align*}
    \epsilon_{\iota_\Lambda^* B}(s \bullet_{(a \mapsto \epsilon_A^{-1}(a) \bullet_B ())^* E_{\R_{\iota_\Lambda^* B}}(U)_0} b)
    &= \iota_\Lambda(x_1 c_1) \bullet_B (s \bullet_{(a \mapsto \epsilon_A^{-1}(a) \bullet_B ())^* E_{\R_{\iota_\Lambda^* B}}(U)_0} b)\\
    ...
    &= (\iota_\Lambda(x_1 (x_{i + 1} c_{n + 1})_i) \bullet_{E_{\R_A}(U)} (\epsilon_A^{-1}(s), \iota_\Lambda(x_1), \dots, \iota_\Lambda(x_n))) \bullet_B b\\
    &= (\epsilon_A^{-1}(s) (x_i c_n)_i) \bullet_B b\\
    &= (\epsilon_A^{-1}(s) (x_i)_i \bullet \iota_\Lambda(x_i c_n)_i) \bullet_B b.
  \end{align*}
  Also,
  \begin{align*}
  ...
  \begin{align*}
    ((\epsilon_A^{-1}(s)) \bullet_{E_{\R_A}(U)} ()) (x_i)_i
    &= ((\lambda x_2, x_1) \bullet_A s) (x_i)_i\\
    &= (\lambda x_3, x_1 x_3 x_2 x_3) \bullet_A ((\lambda x_2, x_1) \bullet_A s, \iota_\Lambda((x_i)_i))\\
    &= (\lambda x_3, x_1 x_2 x_3) \bullet_A (s, \langle \iota_\Lambda(x_i) \rangle_i)\\
    &= (x_1 \circ x_2) \bullet_A (s, U^n)\\
    &= s.
  \end{align*}
\end{proof}
...
\begin{proof}
  ...
  \begin{center}
    \begin{tikzcd}
      ...
      E_{\R_{\iota_{\Lambda}^* B}}(U) \arrow[rr, "h"] & & E_{\R_{\iota_{\Lambda}^* C}}(U)
    \end{tikzcd}
  \end{center}
  Now, using functoriality of the pullback of the theory algebras in the coslice category $ (E_{\R_A}(U) \downarrow \AlgTh) $, and using the previous lemma, we get the following diagram of $ E_{\R_A}(U) $-algebras, which gives us the lift $ \bar h : \Alg{E_{\R_A}(U)}(B, C) $:
  \begin{center}
    \begin{tikzcd}[sep = large]
      (a \mapsto \epsilon_A^{-1}(a) \bullet_B ())^* (E_{\R_{\iota_\Lambda^* B}}(U)_0)
        \arrow[r, "(a \mapsto \epsilon_A^{-1}(a) \bullet ())^* h_0"]
  ...
  \TODO: Show that $ \bar h $ restricts to $ h $.
\end{proof}
