...
\end{lemma}
\begin{proof}
  Take $ B : \Alg{\Lambda_A} $. By Lemma \ref{lem:extensions-algebra-coslice} we can consider $ B $ to be a object in the coslice category $ h : \Alg{\Lambda}(A, B) $. As shown earlier in this chapter, $ h $ induces a morphism $ \bar h: \AlgTh(E(U_A), E(U_B)) $. We then have a $ E(U_A) $-algebra $ \bar h^*(E(U_B)_0) $. Now we need to prove that we have an isomorphism of $ \Lambda_A $-algebras
  \[ ([a \mapsto \epsilon_A^{-1}(a) \bullet (), \iota_{\Lambda_n}]_n \cdot \bar h)^*(E(U_B)_0) \cong B. \]
  Under the equivalence in Lemma \ref{lem:extensions-algebra-coslice}, this pullback of $ E(U_B)_0 $ corresponds to some $ (B^\prime, h^\prime) : (A \downarrow \Alg \Lambda) $. The set of $ B^\prime $ is $ \RAct{B_1}(I, U_B) $, its $ \Lambda $-action is given by
  \[ (f, b) \mapsto \bar h_n(\iota_{\Lambda_n}(f)) \bullet_{E(U_B)} b, \]
  and the morphism $ h^\prime $ is given by $ h^\prime(a) = \bar h(\epsilon_A^{-1}(a)) $. Note that by initiality of $ \Lambda $, the following diagram of algebraic theories commutes
  \begin{center}
    \begin{tikzcd}
      & \Lambda \arrow[ld, "\iota_\Lambda"'] \arrow[rd, "\iota_\Lambda"]\\
      E(U_A) \arrow[rr, "\bar h"] && E(U_B)
    \end{tikzcd}
  \end{center}
  Therefore, $ \bar h_n(\iota_{\Lambda_n}(f)) = \iota_{\Lambda_n}(f) $, so the $ \Lambda $-action on $ B^\prime $ coincides with the action on $ \iota_\Lambda^*(E(U_B)_0) $ and $ B^\prime = \iota_\Lambda^*(E(U_B)_0) $. We have the following diagram in $ \Alg \Lambda $:
  \begin{center}
    \begin{tikzcd}[sep = large]
      & A \arrow[ld, "a \mapsto \bar h(\epsilon_A^{-1}(a))"'] \arrow[rd, "h"]\\
      E(U_B)_0 \arrow[rr, "\epsilon_B"] & & B
    \end{tikzcd}
  \end{center}
  By Lemma \ref{lem:counit-natural}, the diagram commutes, which shows that $ \epsilon_B $ is an isomorphism in the coslice category under $ A $. Pulling this isomorphism back along the equivalence from Lemma \ref{lem:extensions-algebra-coslice} gives an isomorphism of $ \Lambda_A $-algebras
  \[ ([a \mapsto \epsilon_A^{-1}(a) \bullet (), \iota_{\Lambda_n}]_n \cdot \bar h)^*(E(U_B)_0) \cong B \]
  and this concludes the proof.
\end{proof}
