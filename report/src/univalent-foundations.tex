...
\end{lemma}
\begin{proof}
  See \textcite{univalent-categories}, Theorem 8.5.
\end{proof}
...
Also, Theorem 4.5 in \textcite{univalent-categories} shows that if a category $ B $ is univalent (in the paper, categories are called `precategories` and univalent categories are just called `categories'), then the functor category $ A \rightarrow B $ is also univalent. In particular, the category of (pre)sheaves $ A \to \SET $ is univalent.
...
The last structure in this section for which we want to consider the univalence axiom, is the 2-category $ \Cat $ of categories. In general, we cannot show that this satisfies the univalence principle. However, we will restrict our attention to the sub-2-category of univalent categories, which are the categories that we want to study. Then Theorem 6.8 in \textcite{univalent-categories} shows that for univalent categories $ C $ and $ D $, there is an equivalence between $ C = D $ and $ C \simeq D $, where $ C \simeq D $ denotes the type of (adjoint) equivalences of categories (see Definition \ref{def:equivalence-of-categories}).
...
\end{lemma}
\begin{proof}
  See \textcite{univalent-categories}, Lemma 6.8.
\end{proof}
...
\begin{remark}
  ...
  For something to look like a circle or tube, we need this group $ x = x $ to be isomorphic to $ \mathbb Z $. For an example, see \textcite{circle-fundamental-group}.
\end{remark}
