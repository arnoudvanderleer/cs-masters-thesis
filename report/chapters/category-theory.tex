...
\begin{center}
  \begin{tikzcd}
    F(d^\prime)\arrow[rd, "f^\prime"']  \arrow[r, "F(g)"] & F(d) \arrow[d, "f"]\\
    & c
  \end{tikzcd}
\end{center}
...
\begin{definition}
  A morphism $ f: E(y, z) $ is called \iindex{cartesian} if for all $ g: E(x, z) $ and $ w: B(P(x), P(y)) $ with $ w \cdot P(f) = P(g) $, there exists $ \bar w: E(x, y) $ such that $ P(\bar w) = w $ and $ \bar w \cdot f = g $, like illustrated in the following diagram from \cite{nlab:grothendieck_fibration}
  \begin{center}
    \begin{tikzcd}[row sep = .5in]
      E \arrow[d, "P"] &x \arrow[rr, "\forall g", bend left] \arrow[r, "\exists! \bar w"'] & y \arrow[r, "f"'] & z\\
      B & P(x) \arrow[r, "\forall w"'] \arrow[rr, "P(g)", bend left] & P(y) \arrow[r, "P(f)"'] & P(z)
    \end{tikzcd}
  \end{center}
\end{definition}

\begin{definition}
  $ P $ is a \iindex{fibration} if for all $ y: E $ and morphisms $ f: B(x, P(y)) $, there exists a cartesian morphism $ \bar f: E(\bar x, y) $ such that $ P(\bar f) = f $.
\end{definition}
...
\begin{definition}
  ...
  \end{center}
\end{definition}
The \textit{coslice category}\index{slice category!co-} $ c \downarrow C $ is similar, but with the morphisms from $ c $:
...
\begin{definition}
  Given an object $ c $ in a category $ C $ with terminal object $ t $. A \iindex{global element} of $ c $ is a morphism $ C(t, c) $.
\end{definition}
