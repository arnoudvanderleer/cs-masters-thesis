...
An alternative characterization of an adjunction $ L \dashv R $ is as a natural bijection
...
\begin{lemma}
  For $ c, d, d^c : C $, we have a natural bijection
  \[ C(d^\prime \times c, d) \cong C(d^\prime, d^c) \]
  iff we have a universal arrow ...
\end{lemma}
\begin{proof}
  \TODO
\end{proof}
...
These, together with the identity and composition from $ \SET $, constitute a category $ \RAct M $ of right $ M $-actions.
