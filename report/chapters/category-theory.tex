...
\begin{proof}
  ...
  \begin{center}
    \begin{tikzcd}
      C \arrow[r, "\iota_C"] \arrow[d, "Y"] & \R \arrow[d, "Y"]\\
      P C & P \R \arrow[l, "\sim"', "{\op{\iota_C}}_*"]
    \end{tikzcd}
  \end{center}
  ...
  Now, to show that it is essentially surjective, take an object $ F : \hat \R $. That is, take a retract \begin{tikzcd} F \arrow[r, shift left, "s"] & Y(c) \arrow[l, shift left, "r"] \end{tikzcd}. Note that $ r \cdot s $ is an idempotent on $ Y(c) $. Since $ Y $ is a fully faithful embedding, it gives an equivalence on the hom-sets, so we have an element $ e : C(c, c) $ that maps to $ r \cdot s : P C (Y(c), Y(c)) $. We will show that $ (c, e) $ is our preimage in $ \R $ of $ F $.
