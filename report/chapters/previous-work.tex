...
On the other hand, $ \sum_A $ is generally not surjective. To mitigate this somewhat, we can choose our subtype $ D $ of display maps in such a way that the restriction $ \R^A \to (C \downarrow_D A) $ becomes essentially surjective.

\begin{definition}
  For $ \R $, we will consider a morphism $ f: X \to A $ to be a display map if we have some $ Y : \R^A $ like above, and some isomorphism $ g: X \xrightarrow{\sim} \sum_A Y $ such that $ f = g \cdot p_Y $:
  \begin{center}
    \begin{tikzcd}
      X \arrow[rd, "f"] \arrow[rr, "g"] & & \sum_A Y \arrow[ld, "p_1"]\\
      & A
    \end{tikzcd}
  \end{center}
\end{definition}
