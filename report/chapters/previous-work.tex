...
\begin{remark}
  ...
  However, note that we have a weak equivalence (an essentially surjective and fully faithful functor) between strict categories (categories in which the type of objects is a homotopy set) $ \sum_A: \R^A \xrightarrow \sim (\R \downarrow_D A) $. If we assume the axiom of choice, $ \sum_A $ becomes an equivalence of categories (\cite{univalent-categories}, introduction, page 2 \TODO: maybe find a better reference for this) and then our pullback functor is given by
...
\begin{proof}
  ...
  so $ \hat f $ is unique and $ (\alpha_* Y, \epsilon_Y) $ is a universal arrow, which concludes the proof that $ (\alpha \cdot -) \dashv \alpha_* $.
  % \begin{align*}
  %   \hat f
