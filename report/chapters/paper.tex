...
\begin{proof}
  ...
  \begin{center}
    \begin{tikzcd}
      \Delta_X W^Z \arrow[r] \arrow[d] & \Delta_X (Z^Z \times W^Z) \arrow[d, "p_1"]\\
...
%   Hyland then claims that the fact that $ \R $ is cartesian closed relative to this class of display maps (which Taylor shows in his PhD thesis) follows from the theorem above.

%   The proof of this recalls that $ \R $ is a full subcategory of $ \Pshf L $, and that for $ X : \R $, all elements of $ \R(X) $ (i.e. all $ X $-retracts of $ X \times L $) are also elements of $ \R $. This uses Definition \ref{def:karoubi'}, the alternative definition of the category $ \hat \R $ of retracts as the full subcategory of $ P L(1) $ consisting of the retracts of all $ Y(c) : P L(1) $ for $ c: L(1) $.

%   And indeed, we have a weak equivalence $ \R \hookrightarrow \hat R $. One could imagine similar weak equivalences $ (\R \downarrow_D X) \hookrightarrow \R(X) $. One could then attempt, for some $ f: \R(X) $, to pre- and postcompose the functors $ \sum_f $, $ f^* $ and $ \prod_f $ with this equivalence, to get these functors on the relative slices of $ \R $.
%   \begin{center}
%     \begin{tikzcd}
...
%   and its right adjoint $ \R(X) \to \R(Y) $ with these equivalences to get .

%   However, for every $ \hat Y : \hat R $, we only have mere existence of a preimage $ Y : \R $.
% \end{remark}
% we can find an object $  $.

% the definition of $ \hat \R $ as a full subcategory of $ P L(1) \cong \Pshf L $.

% , and $ \iota: \R \hookrightarrow \Pshf L $ the embedding, $ \R(\iota(X)) $ is equivalent to $ (\R \downarrow_D X) $. This uses Definition \ref{def:karoubi'} of $ \hat \R $. By Lemma \ref{lem:retracts-rezk}, $ \R $ and $ \hat \R $ are weakly equivalent.
% \begin{corollary}
% \end{corollary}
% \begin{proof}

%   This stems from the idea that $ \Pshf L \cong P \R $, and that $ \R $ consists exactly of the retracts of $  $.
%   However, without the axiom of choice, it is only equivalent to $ (\hat \R \downarrow_D X) $.
% \end{proof}
