...
\subsubsection{Constructing a theory from an algebra}
...
\begin{lemma}
  ...
  \[ \eta: L \xrightarrow{\sim} E(U_{L_0}). \]
\end{lemma}
\begin{proof}
  ...
  Then $ \eta $ arises by combining this with the isomorphism from Scott's representation theorem (Theorem \ref{thm:representation-theorem})
  \[ \eta: L \xrightarrow \sim E_{\Pshf L}(L) \xrightarrow \sim E_{\RAct{{L_0}_1}}(U_{L_0}). \]
  It is quite easy to work out that we can make this explicit as
  \[ \eta_n:
    L_n
...
\begin{lemma}\label{lem:counit-natural}
  $ \epsilon $ is a natural transformation. I.e., for all $ F: \Alg{\Lambda}(A, B) $, the following diagram commutes:
  \begin{center}
    \begin{tikzcd}
...
Now, Hyland leaves the following implicit:
\begin{lemma}
  $ \epsilon $ and $ \eta $ satisfy the zigzag identities.
\end{lemma}
\begin{proof}

\end{proof}

Finally, we can state the fundamental theorem of the $ \lambda $-calculus:
\begin{theorem}
